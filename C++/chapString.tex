...
public:
    int strStr(const string &haystack, const string &needle) {
        const int N = haystack.length() - needle.length() + 1;
        for (int i = 0; i < N; ++i) {
            int j = 0;
            for (; j < needle.length() && (i + j) < haystack.length(); ++j) {
                if (haystack[i + j] == needle[j]) /*empty*/;
                else break;
            }
...
思路三：Manacher’s Algorithm, 复杂度$O(n)$。详细解释见 \myurl{http://lintcode.com/2011/11/longest-palindromic-substring-part-ii.html} 。
